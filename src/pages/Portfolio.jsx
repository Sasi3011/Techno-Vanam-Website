...
const ProjectCard = ({ project, reverse }) => (
  <a
    ...
    } rounded-2xl sm:rounded-3xl md:rounded-3xl overflow-hidden shadow-sm bg-white hover:shadow-lg transition duration-300`}
  >
    {/* Text Section */}
    <div className="p-4 sm:p-6 md:p-8 md:w-1/2 flex flex-col justify-center">
      <img src={project.logo} alt={`${project.title} logo`} className="w-[100px] sm:w-[120px] md:w-[140px] mb-4 sm:mb-6" />
      <p className="text-[#667097] text-sm sm:text-base md:text-lg leading-relaxed md:leading-7 mb-4 sm:mb-6">
        {project.description}
      </p>
      <span
        className={`font-semibold text-xs sm:text-sm md:text-sm uppercase flex items-center gap-1 sm:gap-2 ${project.linkColor}`}
      >
        See project
        <svg
          className="w-3 sm:w-4 md:w-5 h-3 sm:h-4 md:h-5 transform rotate-90"
          viewBox="0 0 16 16"
          fill="none"
          stroke="currentColor"
    ...
    {/* Image Section */}
    <div className="md:w-1/2 h-[200px] sm:h-[300px] md:h-[400px]">
      <img
        src={project.image}
...
const Portfolio = () => {
  return (
    <>
      <section className="w-full flex flex-col items-center py-8 sm:py-12 md:py-20 px-4 sm:px-6">
        <div className="w-full max-w-[95%] sm:max-w-[90%] md:max-w-4xl lg:max-w-5xl xl:max-w-7xl text-center mb-8 sm:mb-12 md:mb-16">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#19213D]">
            Portfolio
          </h2>
          <p className="text-[#667097] text-sm sm:text-base md:text-lg mt-3 sm:mt-4 md:mt-4 max-w-[640px] mx-auto">
            Our amazing team of designers and developers have produced some remarkable pieces of work.
          </p>
        </div>

        <div className="w-full max-w-[95%] sm:max-w-[90%] md:max-w-4xl lg:max-w-5xl xl:max-w-7xl space-y-6 sm:space-y-8 md:space-y-12">
      ...
      <section className="w-full flex flex-col items-center py-8 sm:py-12 md:py-20 px-4 sm:px-6">
        <div className="w-full max-w-[95%] sm:max-w-[90%] md:max-w-4xl lg:max-w-5xl xl:max-w-[1320px] flex flex-col items-center gap-6 md:gap-10">
          {/* Header */}
          <div className="w-full max-w-[95%] sm:max-w-[580px] text-center">
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-[42px] leading-tight md:leading-[52px] font-bold text-[#19213D]">
              Let’s bring your vision to life
            </h2>
          </div>

          {/* Subheading */}
          <div className="w-full max-w-[95%] sm:max-w-[520px] text-center">
            <p className="text-sm sm:text-base md:text-lg leading-relaxed md:leading-[30px] text-[#667097]">
              Explore how Technovanam builds custom solutions — or reach out and let’s create something exceptional.
            </p>
          </div>

          {/* Cards Container */}
          <div className="w-full flex flex-col sm:flex-row justify-center gap-4 sm:gap-5 md:gap-7">
            {/* Card 1 */}
            <div className="w-full sm:w-[calc(50%-1rem)] max-w-[622px] bg-white border border-[#EBEFF6] rounded-2xl sm:rounded-3xl md:rounded-[32px] shadow-[0px_6px_20px_rgba(20,20,43,0.06)] p-6 sm:p-8 md:p-[55px_41px] flex flex-col min-h-[300px] sm:min-h-[400px] md:min-h-[462px]">
              {/* Logo */}
              <div className="mb-4 sm:mb-6">
                <div className="w-[60px] sm:w-[70px] md:w-[80px] h-[60px] sm:h-[70px] md:h-[80px] flex justify-center items-center mb-4 sm:mb-6 md:mb-[24px]">
                  <img src="https://via.placeholder.com/80" alt="Logo 1" className="w-full h-full object-cover" />
                </div>
              </div>

              {/* Title */}
              <div className="mb-2 sm:mb-3 w-full max-w-[540px]">
                <h3 className="text-xl sm:text-2xl md:text-[32px] leading-tight md:leading-[42px] font-bold text-[#19213D]">
                  Ideas deserve more than just code
                </h3>
              </div>

              {/* Description */}
              <div className="mb-4 sm:mb-6 md:mb-8 w-full max-w-[540px]">
                <p className="text-sm sm:text-base md:text-lg leading-relaxed md:leading-[30px] text-[#667097]">
              ...
              {/* Button */}
              <button className="flex items-center gap-1 px-6 sm:px-8 md:px-[39px] py-3 sm:py-4 md:py-[27px] bg-[#4A3AFF] text-white font-bold text-sm sm:text-base md:text-[18px] rounded-full shadow-[0px_2px_6px_rgba(74,58,255,0.2)] border border-[#4A3AFF] w-full sm:w-[259.81px] h-[56px] sm:h-[64px] md:h-[72px]">
                Let’s Build Together
                <svg
                  className="w-3 sm:w-4 md:w-5 h-3 sm:h-4 md:h-5 transform -scale-y-100"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  xmlns="http://www.w3.org/2000/svg"
            ...
            <div className="w-full sm:w-[calc(50%-1rem)] max-w-[622px] bg-white border border-[#EBEFF6] rounded-2xl sm:rounded-3xl md:rounded-[32px] shadow-[0px_6px_20px_rgba(20,20,43,0.06)] p-6 sm:p-8 md:p-[55px_41px] flex flex-col min-h-[300px] sm:min-h-[400px] md:min-h-[462px]">
              ...
              <div className="mb-4 sm:mb-6">
                <div className="w-[60px] sm:w-[70px] md:w-[80px] h-[60px] sm:h-[70px] md:h-[80px] flex justify-center items-center mb-4 sm:mb-6 md:mb-[24px]">
                  <img src="https://via.placeholder.com/80" alt="Logo 2" className="w-full h-full object-cover" />
                </div>
              </div>
              ...
              {/* Button */}
              <button className="flex items-center gap-1 px-6 sm:px-8 md:px-[39px] py-3 sm:py-4 md:py-[27px] bg-[#2388FF] text-white font-bold text-sm sm:text-base md:text-[18px] rounded-full shadow-[0px_2px_6px_rgba(255,45,70,0.15)] border border-[#2388FF] w-full sm:w-[240px] h-[56px] sm:h-[64px] md:h-[72px]">
                Start Your Journey
                <svg
                  className="w-3 sm:w-4 md:w-5 h-3 sm:h-4 md:h-5 transform -scale-y-100"
                  viewBox="0 0 24 24"
